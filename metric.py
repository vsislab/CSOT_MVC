from sklearn.metrics import normalized_mutual_info_score, adjusted_rand_score, accuracy_score, v_measure_score
...
def inference(loader, model, device, view, data_size):
    ...
    for step, (xs, y, _) in enumerate(loader):
        ...
        h_cat_vector.extend(h_cat.cpu().detach().numpy()) 
        soft_vector.extend(q.cpu().detach().numpy())
        labels_vector.extend(y.numpy())

    h = np.array(h_vector)
    h_cat = np.array(h_cat_vector)
    ...
    return total_pred, pred_vectors, Hs, labels_vector, Zs, h, h_cat
...
def valid(model, device, dataset, view, data_size, class_num, eval_h=False):
    ...
    total_pred, _, _, labels_vector, _, _, _ = inference(test_loader, model, device, view, data_size)
